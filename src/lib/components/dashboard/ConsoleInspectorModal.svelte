...
{#if isOpen}
	...
	>
		...
		>
			...
			<div class="flex-1 overflow-y-auto bg-slate-950 p-6 font-mono text-xs">
				{#if isLoading}
					<div class="flex h-64 flex-col items-center justify-center space-y-3 text-slate-500">
						<span class="loading loading-md loading-spinner text-purple-500"></span>
						<p class="font-sans text-xs">Loading session telemetry logs from R2...</p>
					</div>
				{:else if fetchError}
					<div
						class="rounded-xl border border-rose-500/30 bg-rose-500/10 p-6 text-center text-rose-400"
					>
					...
					<div class="space-y-2">
						{#each filteredLogs() as log, idx (idx)}
							{#if log.event === 'console.error' || log.event === 'error' || log.event === 'unhandledrejection'}
								<div
									class="rounded-xl border border-rose-500/30 bg-rose-950/40 p-4 text-rose-300 shadow-md"
								>
									...
									{#if log.data?.stack}
										<details class="mt-3">
											<summary
												class="cursor-pointer font-sans text-[11px] font-bold text-rose-400 hover:underline"
											>
												View Stack Trace
											</summary>
											<pre
												class="mt-2 overflow-x-auto rounded-lg bg-slate-950 p-3 text-[10px] text-slate-400">{log
													.data.stack}</pre>
										</details>
									{/if}
								</div>
							{:else if log.event === 'console.warn'}
								<div
									class="flex items-start gap-3 rounded-lg border border-amber-500/20 bg-amber-950/20 p-3 text-amber-300"
								>
							...
							{:else if log.event === 'feedback'}
								<div
									class="rounded-xl border border-purple-500/30 bg-purple-950/30 p-4 text-purple-200"
								>
									<div class="flex items-center justify-between">
									...
									<div class="mt-2 flex items-center gap-2">
										<span class="text-xl"
											>{log.data?.sentiment === 'fun'
												? '😀'
												: log.data?.sentiment === 'neutral'
													? '😐'
													: '🙁'}</span
										>
									...
									{#if log.data?.comment}
										<p
											class="mt-2 rounded-lg bg-slate-950/60 p-3 font-sans text-xs text-slate-300 italic"
										>
											"{log.data.comment}"
										</p>
									{/if}
							...
							{:else}
								<div
									class="flex items-start gap-3 rounded-lg border border-slate-800/60 bg-slate-900/40 p-2.5 text-slate-300 hover:bg-slate-900"
								>
									<span class="text-slate-500">[{formatTimestamp(log.timestamp)}]</span>
									<span class="font-bold text-blue-400">{log.event}:</span>
									<span class="flex-1 whitespace-pre-wrap text-slate-200"
										>{log.data?.message || JSON.stringify(log.data)}</span
									>
								</div>
							{/if}
						{/each}
			...
			<!-- Footer -->
			<div
				class="flex items-center justify-between border-t border-slate-800 bg-slate-950/60 p-4 text-xs text-slate-500"
			>
				<span>Showing {filteredLogs().length} log events</span>
				<button
					onclick={onClose}
					class="btn rounded-xl border border-slate-800 bg-slate-900 px-4 font-bold text-slate-300 btn-sm hover:bg-slate-800 hover:text-white"
				>
					Close
				</button>
			</div>
